...
class USAMap extends React.Component {
  ...
  buildPaths = () => {
    ...
    for (let stateKey in dataStates) {
      const path = <USAState key={stateKey} stateName={dataStates[stateKey].name} dimensions={dataStates[stateKey]["dimensions"]} state={dataStates[stateKey].name} fill={this.fillStateColor(stateKey)} onClickState={this.stateClickHandler(dataStates[stateKey].name)} />
      paths.push(path);
    };
    return paths;
  };
